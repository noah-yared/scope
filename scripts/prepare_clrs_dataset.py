...
DEFAULT_SAVE_PATH = Path(__file__).parent / "datasets" / "processed_train.parquet"
...
def prepare_clrs_dataset(fetch_counts: dict[ProblemType, int], num_rows: int, save_path: Path) -> pd.DataFrame:
    """
    ...
    prepared_df.to_parquet(ensure_save_path(save_path))
...
def print_usage() -> None:
    usage = """
python3 prepare_clrs_dataset.py --num-rows | -n <num_rows> [--save-path | -s <save_path>] [--fetch-counts-path | -f <fetch_path>]

    Arguments:
    --num-rows | -n <num_rows> - Maximum number of examples to fetch from the dataset.
    --save-path | -s <save_path> - Path to (parquet) file to save the processed dataset in.
    ...
    Example usage:
    python3 prepare_clrs_dataset.py --num-rows 100 --save-path ./processed_dataset.parquet --fetch-counts-path ./fetch_counts.json
    python3 prepare_clrs_dataset.py --num-rows 100 --save-path ./processed_dataset.parquet # default fetch counts is 100 // 8 = 12 examples per problem type
"""
    print(usage)
...
def parse_args(reqs: tuple[str, ...] = ("num_rows",)) -> dict[str, int | Path | dict[ProblemType, int]]:
    ...
    def apply_defaults(config: dict[str, int | Path | dict[ProblemType, int]]) -> dict[str, int | Path | dict[ProblemType, int]]:
        defaults = {
            "fetch_counts": {pt: config["num_rows"] // len(PROBLEM_TYPES) for pt in PROBLEM_TYPES},
    ...
    for i in range(0, len(args), 2):
        ...
        elif args[i] in ("--save-path", "-s"):
            config["save_path"] = Path(args[i+1]).resolve().with_suffix('.parquet')
...
def main() -> None:
    ...
    print(f"\nSaved the processed dataset as a parquet file to path: {config['save_path']}")
